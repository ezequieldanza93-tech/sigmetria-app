...
export interface DatosProtocoloBase {
  ...
  // Carátula visual (best-effort: si faltan, no se muestran)
  fotoEstablecimiento?: string // data URL de la foto del sitio
  mapaEstablecimiento?: string // data URL del mapa estático (tiles OSM)
  // Medición
  instrumento?: string
  calibracion?: string
...
const SHARED_CSS = `
  ...
  /* Foto del establecimiento + mapa estático en la carátula (mapa izq 62mm / foto der 85mm, 48mm alto) */
  ...
  .cv-sec, .anx-t { position:relative; z-index:1; font-size:13pt; font-weight:700; color:#2E7D33; border-bottom:1px solid #E4E8E4; padding-bottom:3px; margin:8mm 0 6px; }
...
<section class="hoja vert cover">${WM}
  ...
  ${(d.fotoEstablecimiento || d.mapaEstablecimiento) ? `<div class="cv-media">
    ${d.mapaEstablecimiento ? `<div class="cv-media-box mapa"><img src="${d.mapaEstablecimiento}"><span>Ubicación</span></div>` : ''}
    ${d.fotoEstablecimiento ? `<div class="cv-media-box foto"><img src="${d.fotoEstablecimiento}"><span>Establecimiento</span></div>` : ''}
  </div>` : ''}
  <div class="card">
...
export function ensamblarProtocolo<T extends DatosProtocoloBase>(
  ...
  // 4) Carátula. Los anexos REALES (encomienda/plano/certificado/observaciones) se fusionan
  // al final del PDF con pdf-lib (armarPdfFinalConAnexos en el bridge emitir-evidencia-*),
  // con una hoja índice "ANEXOS". Ya NO se muestran cajas placeholder vacías.
