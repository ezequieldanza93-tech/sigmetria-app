...
import { Check, Eye, EyeOff, Globe, Sun, Moon, Loader2, ArrowLeft } from 'lucide-react'
import Link from 'next/link'
import { updatePerfil, updatePassword } from '@/lib/actions/perfil-usuario'
...
const SECTIONS: SectionMeta[] = [
  {
    ...
    shortTitle: 'Perfil',
    description: 'Tu nombre, apellido y email de la cuenta.',
  },
  {
    id: 2,
    title: 'Apariencia',
...
export function PerfilForm({ fullName, email }: Props) {
  ...
  // ── Cálculo de checks (progreso) ──────────────────────────────────────────
  const checks: ProgressCheck[] = [
    { id: 'nombre', label: 'Nombre y apellido', done: name.trim().length > 0, section: 1 },
    { id: 'email', label: 'Email', done: email.length > 0, section: 1 },
  ...
  function handleSaveName(e: React.FormEvent) {
    ...
    startNameTransition(async () => {
      const res = await updatePerfil({ full_name: name })
      if (res?.error) { setNameStatus('error'); setNameMsg(res.error) }
      else { setNameStatus('ok'); setNameMsg('Nombre actualizado correctamente') }
    })
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Ring de progreso global (sticky, igual que establecimiento) */}
      ...
      <FormSection step={1} title={SECTIONS[0].title} description={SECTIONS[0].description} isActive={currentSection === 1} sectionStats={sectionStats[1]}>
        <form onSubmit={handleSaveName} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1.5">Nombre y apellido</label>
            <input
              type="text"
              value={name}
              onChange={e => { setName(e.target.value); setNameStatus('idle') }}
              className="w-full px-3 py-2 text-sm border border-border-default rounded-lg bg-surface-base text-text-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
          ...
          <div>
            ...
            <input
              ...
              value={email}
              readOnly
              className="w-full px-3 py-2 text-sm border border-border-subtle rounded-lg bg-surface-sunken text-text-tertiary cursor-not-allowed"
            />
            <p className="text-[11px] text-text-tertiary mt-1">El email no se puede cambiar desde aquí.</p>
          </div>
          {nameStatus !== 'idle' && (
            <p className={`text-xs flex items-center gap-1.5 ${nameStatus === 'ok' ? 'text-success' : 'text-danger'}`}>
              {nameStatus === 'ok' && <Check size={13} />} {nameMsg}
            </p>
          )}
          <div className="flex items-center justify-between pt-1">
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={namePending || name.trim() === fullName.trim()}
              ...
              >
                {namePending && <Loader2 size={14} className="animate-spin" />}
                Guardar
              </button>
              ...
              {(['es', 'en'] as const).map(code => (
                <button
                  ...
                  className={`flex-1 py-2 text-sm font-medium rounded-lg border transition-colors disabled:opacity-50 ${
                    locale === code ? 'border-brand-primary bg-brand-muted text-brand-primary' : 'border-border-default bg-surface-base text-text-secondary hover:bg-surface-elevated'
                  }`}
                >
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-lg border transition-colors ${
                  theme === 'light' ? 'border-brand-primary bg-brand-muted text-brand-primary' : 'border-border-default bg-surface-base text-text-secondary hover:bg-surface-elevated'
                }`}
              >
                <Sun size={15} /> Claro
              </button>
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-lg border transition-colors ${
                  theme === 'dark' ? 'border-brand-primary bg-brand-muted text-brand-primary' : 'border-border-default bg-surface-base text-text-secondary hover:bg-surface-elevated'
                }`}
              >
                <Moon size={15} /> Oscuro
              </button>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pr-10 text-sm border border-border-default rounded-lg bg-surface-base text-text-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
                placeholder="••••••••"
                required
              />
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={pwNext}
                onChange={e => { setPwNext(e.target.value); setPwStatus('idle') }}
                className="w-full px-3 py-2 pr-10 text-sm border border-border-default rounded-lg bg-surface-base text-text-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
