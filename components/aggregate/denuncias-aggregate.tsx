...
export interface DenunciaAggregateRow {
  id: string
  empresa_id: string
...
export function DenunciasAggregate({
  rows,
  ...
  return (
    <div className="px-6 py-6 space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <h2 className="text-lg font-semibold text-text-primary mr-4">Denuncias</h2>
        {showEmpresaFilter && (
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={columnCount} className="px-3 py-8 text-center text-text-tertiary">
                    Sin denuncias para los filtros activos.
                  </td>
                </tr>
              ) : (
                filtered.map(r => (
                  <tr key={r.id} className="border-t border-border-subtle hover:bg-surface-sunken">
                    <td className="px-3 py-2 text-text-tertiary text-xs whitespace-nowrap">{fmt(r.fecha_denuncia)}</td>
                    ...
                    <td className="px-3 py-2 text-text-tertiary text-xs">
                      {r.tipo_denuncia}
                      <span className="ml-2 text-[10px] text-text-tertiary">({r.denunciante_tipo})</span>
                    </td>
                    ...
                    <td className="px-3 py-2 text-text-primary truncate max-w-[20rem]">
                      <Link href={`/dashboard/denuncias/${r.id}`} className="hover:underline">
                        {r.titulo}
                      </Link>
                    </td>
                  </tr>
